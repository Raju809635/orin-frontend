...
export async function pickAndUploadResumeFile(): Promise<{ url: string; fileName: string } | null> {
  let DocumentPicker: any;
  try {
    DocumentPicker = await import("expo-document-picker");
  } catch {
    throw new Error("Resume upload requires latest app build. Please install updated APK.");
  }
  ...
  const uri = asset.uri;
  const fileName = asset.name || `resume-${Date.now()}.pdf`;
